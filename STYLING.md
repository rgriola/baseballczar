...
# Baseball Czar — Styling Conventions

This document is the single source of truth for visual design decisions across the web app (`apps/web`) and any future iOS screens that mirror the web UI.
...
- Avoid arbitrary Tailwind values (e.g., `text-[13px]`, `bg-[#abc123]`). If you need a new value, add it to the config.

---

## iOS / React Native Notes

- React Native Skia uses the same hex format. Define colors in a shared `theme.ts` constants file (to be created during Phase C/D).
- Font families on iOS will map to system fonts until the Geist fonts are bundled — document this when Phase B begins.
