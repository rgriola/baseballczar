...
-- Purpose: Adds game sim provenance columns and atomic scheduled-game persistence RPC.

alter table public.games
  add column if not exists sim_seed int,
  add column if not exists sim_version text,
  add column if not exists sim_config_version text;

comment on column public.games.sim_seed is 'Deterministic RNG seed used to simulate this game';
comment on column public.games.sim_version is 'Scheduled simulation implementation version used for this game';
comment on column public.games.sim_config_version is 'Simulation config calibration version used for this game';
