...
export function buildGameEventRows(result: GameResult): Record<string, unknown>[] {
...
export async function insertGameRecord(
  ...
  const { data: game, error: gameErr } = await supabase
    .from('games')
    .insert(buildGameInsertRow(result, opts))
    .select('id')
    .single();
  ...
  if (result.events.length > 0) {
    const eventInserts = buildGameEventRows(result);
